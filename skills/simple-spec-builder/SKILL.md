...
**Update rule:** The user will indicate their choice simply by marking it with `[x]` (or filling in `Other:`). Once the user has marked their choices, the agent must automatically parse these selections, reflect the changes in the corresponding specifications, and fill in the decisions in the final summary table under the heading `## Summary (Only for Agent, do not edit this table manually)`. The user must not edit this table manually.
